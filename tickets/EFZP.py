...
#under the License.
"""
from https://github.com/Trindaz/EFZP (Copyright 2011 Chorus Technologies Inc) with some modifications
...
def get_signature(email_text):
    ...
    salutation = get_salutation(email_text)
    if salutation: email_text = email_text[email_text.find(salutation) + len(salutation):]
...
def get_body(email_text, check_salutation=True, check_signature=True, check_reply_text=True):
    ...
    if check_salutation:
        ...
        if sal: email_text = email_text[email_text.find(sal)+len(sal):]
...
def get_salutation(email_text):
    ...
    groups = re.search(pattern, email_text, re.IGNORECASE)
    salutation = None
    if not groups is None:
